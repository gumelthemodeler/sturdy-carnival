...
task.spawn(function()
	...
	if type(CosmeticData.CheckUnlock) == "function" then
		for key, data in pairs(CosmeticData.Titles or {}) do 
			UnlockedCosmeticsCache.Titles[key] = CosmeticData.CheckUnlock(player, data.ReqType, data.ReqValue) 
		end
		for key, data in pairs(CosmeticData.Auras or {}) do 
			UnlockedCosmeticsCache.Auras[key] = CosmeticData.CheckUnlock(player, data.ReqType, data.ReqValue) 
		end
	end
end)
...
local function BuildIdentityTab(parentFrame, cachedTooltipMgr)
	...
	local function BuildCosmeticList(tab, typeKey, dataPool)
		...
		for _, item in ipairs(sorted) do
			...
			local desc = CreateSharpLabel(card, item.Data.Desc, UDim2.new(1, -110, 0, 45), Enum.Font.GothamMedium, Color3.fromRGB(160, 160, 175), 12)
			desc.Position = UDim2.new(0, 15, 0, 35)
			desc.TextWrapped = true
			desc.TextXAlignment = Enum.TextXAlignment.Left
			desc.TextYAlignment = Enum.TextYAlignment.Top

			local btn, btnStroke = CreateSharpButton(card, "", UDim2.new(0, 90, 0, 40), Enum.Font.GothamBlack, 12)
			...
			local function UpdateState()
				...
				if type(CosmeticData.CheckUnlock) == "function" then 
					isUnlocked = CosmeticData.CheckUnlock(player, item.Data.ReqType, item.Data.ReqValue) 
				end
				...
				if isEquipped then 
					btn.Text = "EQUIPPED"
					btn.TextColor3 = Color3.fromRGB(225, 185, 60)
					btnStroke.Color = Color3.fromRGB(225, 185, 60)
				elseif isUnlocked then 
					...
					btn.Text = "LOCKED"
					btn.TextColor3 = Color3.fromRGB(70, 70, 80)
					btnStroke.Color = Color3.fromRGB(70, 70, 80) 
				end
			end
	...
	local function RenderRadarChart()
		...
		local w = RadarContainer.AbsoluteSize.X
		local h = RadarContainer.AbsoluteSize.Y
		...
		local maxVal = 100
		if type(GameData) == "table" and type(GameData.GetStatCap) == "function" then 
			maxVal = GameData.GetStatCap(p and p.Value or 0) 
		else 
			maxVal = 100 + ((p and p.Value or 0) * 10) 
		end

		local stats = isShowingTitanStats and { 
			{Name = "POW", Val = player:GetAttribute("Titan_Power_Val") or 1}, 
			{Name = "SPD", Val = player:GetAttribute("Titan_Speed_Val") or 1}, 
			{Name = "HRD", Val = player:GetAttribute("Titan_Hardening_Val") or 1}, 
			{Name = "END", Val = player:GetAttribute("Titan_Endurance_Val") or 1}, 
			{Name = "STM", Val = player:GetAttribute("Titan_Precision_Val") or 1}, 
			{Name = "POT", Val = player:GetAttribute("Titan_Potential_Val") or 1} 
		} or { 
			{Name = "HP", Val = player:GetAttribute("Health") or 1}, 
			{Name = "STR", Val = player:GetAttribute("Strength") or 1}, 
			{Name = "DEF", Val = player:GetAttribute("Defense") or 1}, 
			{Name = "SPD", Val = player:GetAttribute("Speed") or 1}, 
			{Name = "GAS", Val = player:GetAttribute("Gas") or 1}, 
			{Name = "RES", Val = player:GetAttribute("Resolve") or 1} 
		}
		...
		for ring = 1, 3 do 
			...
			for i = 1, 6 do 
				local nextI = i % 6 + 1
				DrawLineScale(RadarContainer, centerX + r*math.cos(math.rad(angles[i])), centerY + r*math.sin(math.rad(angles[i])), centerX + r*math.cos(math.rad(angles[nextI])), centerY + r*math.sin(math.rad(angles[nextI])), Color3.fromRGB(60, 60, 70), 1, 1) 
			end 
		end

		for i = 1, 6 do 
			local rad = math.rad(angles[i])
			local px = centerX + maxRadius * math.cos(rad)
			local py = centerY + maxRadius * math.sin(rad)
			DrawLineScale(RadarContainer, centerX, centerY, px, py, Color3.fromRGB(60, 60, 70), 1, 1)

			local lbl = CreateSharpLabel(RadarContainer, stats[i].Name .. "\n" .. stats[i].Val, UDim2.new(0, 30, 0, 15), Enum.Font.GothamBold, Color3.fromRGB(160, 160, 175), 10)
		...
		for i = 1, 6 do 
			local nextI = i % 6 + 1
			DrawLineScale(RadarContainer, pts[i].X, pts[i].Y, pts[nextI].X, pts[nextI].Y, statColor, 2, 5)
			DrawUITriangle(RadarContainer, Vector2.new(centerX, centerY), pts[i], pts[nextI], statColor, 0.5, 3) 
		end
	end
	...
	toggleStatsBtn.MouseButton1Click:Connect(function() 
		isShowingTitanStats = not isShowingTitanStats
	...
	RefreshProfile = function()
		...
		if cName == "Ackerman" or cName == "Awakened Ackerman" then 
			titanLabel.Text = "Titan: <font color='#FF5555'>(Titan Disabled)</font>" 
		else 
			titanLabel.Text = "Titan: <font color='#FF5555'>" .. tName .. "</font>" 
		end
		...
		if type(CosmeticData) == "table" and CosmeticData.Titles then
			...
			if not resolvedTitleData then 
				for k, v in pairs(CosmeticData.Titles) do 
					if v.Name == pTitle then resolvedTitleData = v break end 
				end 
			end
		end
		if resolvedTitleData then 
			AvatarTitle.Text = string.upper(resolvedTitleData.Name)
			AvatarTitle.TextColor3 = Color3.fromHex((resolvedTitleData.Color or "#FFFFFF"):gsub("#", "")) 
		end
		...
		if type(CosmeticData) == "table" and CosmeticData.Auras then
			...
			if not resolvedAuraData then 
				for k, v in pairs(CosmeticData.Auras) do 
					if v.Name == pAura then resolvedAuraData = v break end 
				end 
			end
		end
		if UIAuraManager and type(UIAuraManager.ApplyAura) == "function" and resolvedAuraData then 
			UIAuraManager.ApplyAura(AvatarAuraGlow, resolvedAuraData, AvatarBox) 
		end
			...
			for iName, iData in pairs(ItemData.Equipment or {}) do 
				...
				if count > 0 then 
					...
					if currentInvFilter == "All" or currentInvFilter == "Gear" then 
						table.insert(inventoryItems, {Name = iName, Data = iData, Count = count}) 
					end 
				end
			end
		...
		table.sort(inventoryItems, function(a, b) 
			local rA = RarityOrder[a.Data.Rarity or "Common"] or 7
			local rB = RarityOrder[b.Data.Rarity or "Common"] or 7
			if rA == rB then return a.Name < b.Name else return rA < rB end 
		end)

		local layoutOrderCounter = 1
		for _, item in ipairs(inventoryItems) do
			local card, cStroke = CreateGrimPanel(InvGrid)
			card.Name = "ItemCard"
			...
			cStroke.Color = isLocked and UIHelpers.Colors.Gold or rarityRGB
			...
			countBadge.BackgroundColor3 = Color3.fromRGB(18, 18, 22)
			countBadge.BorderSizePixel = 1
			countBadge.BorderColor3 = rarityRGB
			countBadge.ZIndex = 3

			local countTag = CreateSharpLabel(countBadge, "x" .. item.Count, UDim2.new(1, 0, 1, 0), Enum.Font.GothamBlack, Color3.fromRGB(245, 245, 245), 11)
			countTag.ZIndex = 4

			if isLocked then 
				local lockIcon = UIHelpers.CreateLabel(card, "🔒", UDim2.new(0, 15, 0, 15), Enum.Font.GothamBlack, UIHelpers.Colors.Gold, 14)
				lockIcon.Position = UDim2.new(0, 5, 0, 5)
				lockIcon.ZIndex = 4 
			end

			local nameLbl = CreateSharpLabel(card, item.Name, UDim2.new(0.88, 0, 0.5, 0), Enum.Font.GothamBold, Color3.fromRGB(245, 245, 245), 12)
			nameLbl.Position = UDim2.new(0.5, 0, 0.5, 2)
			nameLbl.AnchorPoint = Vector2.new(0.5, 0.5)
			nameLbl.TextScaled = true
			nameLbl.TextWrapped = true
			nameLbl.ZIndex = 3
			local tCon2 = Instance.new("UITextSizeConstraint", nameLbl)
			tCon2.MaxTextSize = 12
			tCon2.MinTextSize = 8

			local rarityTag = CreateSharpLabel(card, string.sub(rarityKey, 1, 1), UDim2.new(0, 16, 0, 16), Enum.Font.GothamBlack, rarityRGB, 12)
			...
			if not item.Data.IsGift then
				...
				local function MakeOverlayBtn(text) 
					local obtn, _ = CreateSharpButton(ActionsOverlay, text, UDim2.new(0.9, 0, 0, 24), Enum.Font.GothamBlack, 11)
					obtn.ZIndex = 11
					return obtn 
				end
				...
				if item.Data.Type ~= nil then 
					...
					if isEq then 
						...
						equipBtn.TextColor3 = Color3.fromRGB(255, 100, 100) 
					else 
						equipBtn.Text = "EQUIP"
				...
				elseif item.Data.Action ~= nil then 
					equipBtn.Text = "USE"
					equipBtn.TextColor3 = Color3.fromRGB(200, 150, 255)
					equipBtn.MouseButton1Click:Connect(function() 
						buttonConsumed = true
				...
				sellBtn.MouseButton1Click:Connect(function() 
					buttonConsumed = true
					Network.SellItem:FireServer(item.Name, false)
					ActionsOverlay.Visible = false 
				end)

				lockBtn.MouseButton1Click:Connect(function() 
					buttonConsumed = true
					Network:WaitForChild("ToggleLock"):FireServer(item.Name)
					ActionsOverlay.Visible = false 
				end)
		...
		if currentSlotsUsed >= MAX_INVENTORY_CAPACITY then 
			InvTitle.TextColor3 = Color3.fromRGB(255, 100, 100) 
		else 
			InvTitle.TextColor3 = Color3.fromRGB(225, 185, 60) 
		end
	end
...
local function BuildAttributesTab(parentFrame)
	...
	local padding = Instance.new("UIPadding", MainScroll)
	padding.PaddingTop = UDim.new(0, 15)
	padding.PaddingBottom = UDim.new(0, 25)

	local TrainContainer = Instance.new("Frame", MainScroll)
	TrainContainer.Size = UDim2.new(0.95, 0, 0, 180)
	TrainContainer.BackgroundTransparency = 1
	TrainContainer.LayoutOrder = 1

	local tcLayout = Instance.new("UIListLayout", TrainContainer)
	tcLayout.FillDirection = Enum.FillDirection.Horizontal
	tcLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center
	tcLayout.Padding = UDim.new(0.05, 0)
	...
	local function SafeGetStatCap(prestige) 
		if type(GameData) == "table" and type(GameData.GetStatCap) == "function" then return GameData.GetStatCap(prestige) end
		return 100 + ((prestige or 0) * 10) 
	end

	local function SafeCalculateStatCost(currentStat, baseStat, prestige) 
		if type(GameData) == "table" and type(GameData.CalculateStatCost) == "function" then return GameData.CalculateStatCost(currentStat, baseStat, prestige) end
		local baseCost = 10; local growthFactor = 1.05; local prestigeMultiplier = math.max(0.1, 1 - ((prestige or 0) * 0.03)); local statDifference = math.max(0, (currentStat or 10) - (baseStat or 10))
		return math.floor(baseCost * (growthFactor ^ statDifference) * prestigeMultiplier) 
	end

	local function ParseStat(rawStat) 
		local val = tonumber(rawStat); if val then return val end
		if type(rawStat) == "string" and type(GameData) == "table" and GameData.TitanRanks and GameData.TitanRanks[rawStat] then return GameData.TitanRanks[rawStat] end
		return 10 
	end
	...
	local function GetCombinedBonus(statName)
		...
		if type(GameData) == "table" and GameData.WeaponBonuses and GameData.WeaponBonuses[style] and GameData.WeaponBonuses[style][statName] then 
			bonus += GameData.WeaponBonuses[style][statName] 
		end
		return bonus
	end
	...
	local function GetUpgradeCosts(currentStat, cleanName, prestige) 
		local base = 10
		if prestige == 0 then 
			if type(GameData) == "table" and type(GameData.BaseStats) == "table" and GameData.BaseStats[cleanName] then base = GameData.BaseStats[cleanName] end 
		else 
			base = prestige * 5 
		end
	...
	local function CreateTrainBox(isTitan)
		local box, _ = CreateGrimPanel(TrainContainer)
		box.Size = UDim2.new(0.47, 0, 1, 0)
		box.ClipsDescendants = true

		local title = CreateSharpLabel(box, isTitan and "TITAN TRAINING" or "SOLDIER TRAINING", UDim2.new(1, -20, 0, 30), Enum.Font.Garamond, isTitan and Color3.fromRGB(255, 100, 100) or Color3.fromRGB(200, 200, 220), 18)
		title.Position = UDim2.new(0, 10, 0, 10)
		title.TextXAlignment = Enum.TextXAlignment.Left

		local comboLbl = CreateSharpLabel(box, "", UDim2.new(1, -20, 0, 30), Enum.Font.GothamBlack, isTitan and Color3.fromRGB(255, 150, 100) or Color3.fromRGB(150, 255, 100), 18)
		comboLbl.Position = UDim2.new(0, 10, 0, 40)
		comboLbl.TextXAlignment = Enum.TextXAlignment.Left
		comboLbl.Visible = false
		comboLbl.RichText = true

		local missBtn = Instance.new("TextButton", box)
		missBtn.Size = UDim2.new(1, 0, 1, 0)
		missBtn.BackgroundTransparency = 1
		missBtn.Text = ""

		local tBtn, _ = CreateSharpButton(box, isTitan and "TRAIN TITAN" or "TRAIN SOLDIER", UDim2.new(0.5, 0, 0, 60), Enum.Font.GothamBlack, 16)
		tBtn.AnchorPoint = Vector2.new(0.5, 0.5)
		tBtn.Position = UDim2.new(0.5, 0, 0.6, 0)
		tBtn.TextScaled = true
		Instance.new("UITextSizeConstraint", tBtn).MaxTextSize = 16

		if isTitan then 
			tBtn.TextColor3 = Color3.fromRGB(255, 100, 100) 
		else 
			tBtn.TextColor3 = Color3.fromRGB(100, 255, 100) 
		end

		local function CreateFloatingText(textStr, color, startPos)
			local fTxt = CreateSharpLabel(box, textStr, UDim2.new(0, 100, 0, 30), Enum.Font.GothamBlack, color, 24)
	...
	CreateTrainBox(false)
	CreateTrainBox(true)

	local ColumnsContainer = Instance.new("Frame", MainScroll)
	ColumnsContainer.Size = UDim2.new(0.95, 0, 0, 0)
	...
	ccLayout.Padding = UDim.new(0, 20)
	...
	local function CreateStatRow(statName, parent, isTitan, layoutOrder, amtInput)
		...
		local statLabel = CreateSharpLabel(row, "", UDim2.new(0.45, 0, 1, 0), Enum.Font.GothamBold, isTitan and Color3.fromRGB(255, 100, 100) or Color3.fromRGB(220, 220, 220), 14)
		...
		btnContainer.Size = UDim2.new(0.55, 0, 1, 0)
		...
		local bAdd, addStroke = CreateSharpButton(btnContainer, "+", UDim2.new(0, 60, 0, 36), Enum.Font.GothamBlack, 16)
		local bMax, maxStroke = CreateSharpButton(btnContainer, "MAX", UDim2.new(0, 75, 0, 36), Enum.Font.GothamBlack, 14)
		...
		local function TryUpgrade(amt)
			...
			if added > 0 then 
				...
				if NotificationManager and type(NotificationManager.Show) == "function" then NotificationManager.Show("Not enough XP!", "Error") end
				isUpgrading = false 
			end
		end

		bAdd.MouseButton1Down:Connect(function() 
			local customAmt = tonumber(amtInput.Text) or 1
			if customAmt < 1 then customAmt = 1 end
			TryUpgrade(math.floor(customAmt)) 
		end)
	...
	local function SetupPanel(titleTxt, statList, isTitan, parent)
		local panel, _ = CreateGrimPanel(parent)
		...
		local title = CreateSharpLabel(header, titleTxt, UDim2.new(0.5, 0, 1, 0), Enum.Font.GothamBlack, isTitan and Color3.fromRGB(255, 100, 100) or Color3.fromRGB(200, 200, 220), 18)
		title.TextXAlignment = Enum.TextXAlignment.Left
		...
		local allBtn, _ = CreateSharpButton(controls, "ALL", UDim2.new(0, 60, 0, 30), Enum.Font.GothamBold, 12)
		local amtInput = Instance.new("TextBox", controls)
		amtInput.Size = UDim2.new(0, 50, 0, 30)
		amtInput.Text = "1"
		amtInput.Font = Enum.Font.GothamBold
		amtInput.TextColor3 = Color3.new(1,1,1)
		amtInput.TextSize = 14
		CreateGrimPanel(amtInput)

		local ptsLbl = CreateSharpLabel(controls, "0 XP", UDim2.new(0, 100, 0, 30), Enum.Font.GothamMedium, isTitan and Color3.fromRGB(255, 100, 100) or Color3.fromRGB(100, 255, 100), 12)
		...
		allBtn.MouseButton1Down:Connect(function()
			...
			if totalUpgrades > 0 then 
				...
				if NotificationManager and type(NotificationManager.Show) == "function" then NotificationManager.Show("Not enough XP to upgrade anything!", "Error") end
				isSpammingAll = false 
			end
		end)
		...
		for _, statName in ipairs(allStats) do
			...
			if data then
				...
				if val >= statCap then
					...
					data.BtnAdd.TextColor3 = UIHelpers.Colors.BorderMuted
					data.AddStroke.Color = UIHelpers.Colors.BorderMuted
					data.BtnMax.TextColor3 = UIHelpers.Colors.BorderMuted
					data.MaxStroke.Color = UIHelpers.Colors.BorderMuted
				else
					...
					local function toggle(btn, stroke, canAfford) 
						if canAfford then 
							...
							btn.TextColor3 = UIHelpers.Colors.BorderMuted
	...
	player.AttributeChanged:Connect(function(attr) 
		if table.find(playerStatsList, attr) or table.find(titanStatsList, attr) or attr == "XP" or attr == "TitanXP" or attr == "Titan" then UpdateStats() end 
	end)
	task.spawn(function() 
...
local function BuildSkillsTab(parentFrame)
	...
	local libLayout = Instance.new("UIListLayout", MainScroll)
	...
	local libPad = Instance.new("UIPadding", MainScroll)
	libPad.PaddingTop = UDim.new(0, 15)
	libPad.PaddingBottom = UDim.new(0, 20)
	...
	for i = 1, 4 do
		...
		Instance.new("UIAspectRatioConstraint", slotFrame).AspectRatio = 1.0 
	...
	local sep = Instance.new("Frame", MainScroll)
	sep.Size = UDim2.new(0.95, 0, 0, 2)
	sep.BackgroundColor3 = Color3.fromRGB(70, 70, 80)
	sep.BorderSizePixel = 0
	sep.LayoutOrder = 2

	local LibHeader = CreateSharpLabel(MainScroll, "SKILL LIBRARY", UDim2.new(0.95, 0, 0, 30), Enum.Font.Garamond, Color3.fromRGB(245, 245, 245), 24)
	...
	local function RefreshSkills()
		for _, c in ipairs(MainScroll:GetChildren()) do 
			if c:IsA("Frame") and c.Name == "SkillGridContainer" or (c:IsA("TextLabel") and c.Name == "CatHeader") then 
				c:Destroy() 
			end 
		end
		...
		table.sort(sortedCats, function(a, b) 
			...
			if aUnlocked ~= bUnlocked then return aUnlocked and not bUnlocked end
			return a < b 
		end)
		...
		for _, catName in ipairs(sortedCats) do
			...
			if #skills > 0 then
				local catHeader = CreateSharpLabel(MainScroll, "- " .. catName .. " -", UDim2.new(0.95, 0, 0, 30), Enum.Font.GothamBlack, Color3.fromRGB(225, 185, 60), 16)
				catHeader.Name = "CatHeader"
				catHeader.LayoutOrder = sOrderCount
				sOrderCount += 1
				catHeader.TextXAlignment = Enum.TextXAlignment.Left

				local GridContainer = Instance.new("Frame", MainScroll)
				GridContainer.Name = "SkillGridContainer"
				GridContainer.BackgroundTransparency = 1
				GridContainer.LayoutOrder = sOrderCount
				sOrderCount += 1 

				local uigrid = Instance.new("UIGridLayout", GridContainer)
				uigrid.CellSize = UDim2.new(0, 180, 0, 220)
				uigrid.CellPadding = UDim2.new(0, 15, 0, 20)
				...
				for i, item in ipairs(skills) do
					...
					local sCard, _ = CreateGrimPanel(GridContainer)
					sCard.LayoutOrder = i

					local wepText = ""
					if sData.Requirement == "ODM" then wepText = "[ BASE SKILL ]" else wepText = "[ REQ: " .. string.upper(sData.Requirement) .. " ]" end
					...
					sTitle.Position = UDim2.new(0.5, 0, 0, 10); sTitle.AnchorPoint = Vector2.new(0.5, 0); sTitle.TextWrapped = true; sTitle.TextScaled = true
					...
					desc.Position = UDim2.new(0.5, 0, 0, 68); desc.AnchorPoint = Vector2.new(0.5, 0); desc.TextWrapped = true; desc.TextYAlignment = Enum.TextYAlignment.Top

					local eqBtn, eqStroke = CreateSharpButton(sCard, "EQUIP", UDim2.new(1, -20, 0, 35), Enum.Font.GothamBlack, 12)
					eqBtn.Position = UDim2.new(0.5, 0, 1, -10); eqBtn.AnchorPoint = Vector2.new(0.5, 1)

					if sData.ComboReq then
						local syn = CreateSharpLabel(sCard, "Synergy: After " .. string.upper(sData.ComboReq), UDim2.new(1, -10, 0, 30), Enum.Font.GothamBold, Color3.fromRGB(225, 185, 60), 10)
						syn.Position = UDim2.new(0.5, 0, 1, -48); syn.AnchorPoint = Vector2.new(0.5, 1); syn.TextWrapped = true
					end
					...
					if isEquipped then 
						eqBtn.Text = "EQUIPPED"
						eqBtn.TextColor3 = Color3.fromRGB(225, 185, 60)
						eqStroke.Color = Color3.fromRGB(225, 185, 60)
					elseif not hasWep then 
						eqBtn.Text = "LOCKED"
						eqBtn.TextColor3 = Color3.fromRGB(100, 100, 100)
						eqStroke.Color = Color3.fromRGB(70, 70, 80)
					else 
						eqBtn.Text = "EQUIP"
						eqBtn.TextColor3 = Color3.fromRGB(245, 245, 245)
						eqStroke.Color = Color3.fromRGB(70, 70, 80) 
					end
					...
					if hasWep then
						...
						ActionsOverlay.Size = UDim2.new(1, 0, 1, 0)
						ActionsOverlay.BackgroundColor3 = Color3.fromRGB(18, 18, 22)
						ActionsOverlay.BackgroundTransparency = 0.1
						ActionsOverlay.Visible = false
						ActionsOverlay.ZIndex = 10
						ActionsOverlay.Active = true
						ActionsOverlay.BorderSizePixel = 0
						...
						for sIndex = 1, 4 do
							local slotBtn, _ = CreateSharpButton(ActionsOverlay, "SLOT " .. sIndex, UDim2.new(0.8, 0, 0, 30), Enum.Font.GothamBlack, 12)
							slotBtn.ZIndex = 11
						...
						local closeBtn, _ = CreateSharpButton(ActionsOverlay, "CANCEL", UDim2.new(0.8, 0, 0, 30), Enum.Font.GothamBlack, 12)
						closeBtn.ZIndex = 11
						...
						eqBtn.MouseButton1Click:Connect(function() 
							if ActionsOverlay.Visible then 
								...
								for _, sc in ipairs(MainScroll:GetDescendants()) do 
									if sc.Name == "ActionsOverlay" then sc.Visible = false end 
								end
								ActionsOverlay.Visible = true 
							end 
						end) 
...
local function BuildPrestigeTab(parentFrame)
	...
	local function CreateStatCard(parent, title, valueStr, themeColorHex)
		local card, stroke = CreateGrimPanel(parent)
		card.Size = UDim2.new(0, 160, 1, 0)
		stroke.Color = Color3.fromHex(themeColorHex:gsub("#", ""))
		stroke.Transparency = 0.5

		local tLbl = CreateSharpLabel(card, string.upper(title), UDim2.new(1, 0, 0, 20), Enum.Font.GothamBold, UIHelpers.Colors.TextMuted, 12)
		tLbl.Position = UDim2.new(0, 0, 0, 5)
		local vLbl = CreateSharpLabel(card, valueStr, UDim2.new(1, 0, 0, 30), Enum.Font.GothamBlack, Color3.fromHex(themeColorHex:gsub("#", "")), 22)
	...
	local msLayout = Instance.new("UIListLayout", MainScroll)
	msLayout.Padding = UDim.new(0, 20)
	msLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center

	local msPad = Instance.new("UIPadding", MainScroll)
	msPad.PaddingTop = UDim.new(0, 10)
	msPad.PaddingBottom = UDim.new(0, 20)

	local LeftPanel, _ = CreateGrimPanel(MainScroll)
	LeftPanel.Size = UDim2.new(0.95, 0, 0, 340)
	...
	local ATitle = CreateSharpLabel(LeftPanel, "PRESTIGE ASCENSION", UDim2.new(1, -20, 0, 30), Enum.Font.Garamond, UIHelpers.Colors.Gold, 24)
	ATitle.Position = UDim2.new(0, 10, 0, 15)
	ATitle.TextXAlignment = Enum.TextXAlignment.Left

	local ASub = CreateSharpLabel(LeftPanel, "TIER " .. pLevel .. " ➔ TIER " .. nextLevel, UDim2.new(1, -20, 0, 20), Enum.Font.GothamBold, Color3.fromRGB(150, 255, 150), 16)
	ASub.Position = UDim2.new(0, 10, 0, 50)
	ASub.TextXAlignment = Enum.TextXAlignment.Left

	local RBox = Instance.new("Frame", LeftPanel)
	RBox.Size = UDim2.new(1, -20, 0, 130)
	RBox.Position = UDim2.new(0, 10, 0, 85)
	RBox.BackgroundColor3 = Color3.fromRGB(22, 22, 26)
	local rStroke = Instance.new("UIStroke", RBox)
	rStroke.Color = Color3.fromRGB(70, 70, 80)
	rStroke.Thickness = 2

	local rTitle = CreateSharpLabel(RBox, "NEXT TIER REWARDS:", UDim2.new(1, -10, 0, 25), Enum.Font.GothamBold, UIHelpers.Colors.TextWhite, 14)
	rTitle.Position = UDim2.new(0, 10, 0, 5)
	rTitle.TextXAlignment = Enum.TextXAlignment.Left

	local ptsGain = 5 + (pLevel * 2)
	local r1 = CreateSharpLabel(RBox, "• +" .. ptsGain .. " Prestige Points", UDim2.new(1, -10, 0, 25), Enum.Font.GothamMedium, UIHelpers.Colors.Gold, 13)
	r1.Position = UDim2.new(0, 10, 0, 35)
	r1.TextXAlignment = Enum.TextXAlignment.Left

	local r2 = CreateSharpLabel(RBox, "• Increased Maximum Stat Caps", UDim2.new(1, -10, 0, 25), Enum.Font.GothamMedium, Color3.fromRGB(100, 255, 100), 13)
	r2.Position = UDim2.new(0, 10, 0, 60)
	r2.TextXAlignment = Enum.TextXAlignment.Left

	local r3 = CreateSharpLabel(RBox, "• +20% HP & Damage Multiplier", UDim2.new(1, -10, 0, 25), Enum.Font.GothamMedium, Color3.fromRGB(255, 100, 100), 13)
	r3.Position = UDim2.new(0, 10, 0, 85)
	r3.TextXAlignment = Enum.TextXAlignment.Left

	local WBox = Instance.new("Frame", LeftPanel)
	WBox.Size = UDim2.new(1, -20, 0, 50)
	WBox.Position = UDim2.new(0, 10, 0, 225)
	WBox.BackgroundColor3 = Color3.fromRGB(30, 10, 10)
	local wStroke = Instance.new("UIStroke", WBox)
	wStroke.Color = Color3.fromRGB(150, 50, 50)
	wStroke.Thickness = 2

	local wText = CreateSharpLabel(WBox, "WARNING: Ascending completely resets your Level and Stats.", UDim2.new(1, -20, 1, 0), Enum.Font.GothamBold, Color3.fromRGB(255, 150, 150), 12)
	wText.Position = UDim2.new(0, 10, 0, 0)
	wText.TextWrapped = true
	wText.TextXAlignment = Enum.TextXAlignment.Center

	local AscendBtn, aStroke = CreateSharpButton(LeftPanel, "ASCEND TO TIER " .. nextLevel, UDim2.new(1, -20, 0, 50), Enum.Font.GothamBlack, 18)
	AscendBtn.Position = UDim2.new(0, 10, 1, -60)
	AscendBtn.BackgroundColor3 = Color3.fromRGB(100, 20, 20)
	AscendBtn.TextColor3 = Color3.fromRGB(255, 200, 200)
	aStroke.Color = Color3.fromRGB(255, 50, 50)

	local RightPanel = Instance.new("Frame", MainScroll)
	RightPanel.Size = UDim2.new(0.95, 0, 0, 600)
	RightPanel.BackgroundTransparency = 1

	local PointsLabel = CreateSharpLabel(RightPanel, "AVAILABLE POINTS: 0", UDim2.new(1, 0, 0, 30), Enum.Font.GothamBlack, Color3.fromRGB(150, 255, 150), 18)
	...
	TreeContainer.Size = UDim2.new(1, 0, 1, -190)
	...
	local DetailPanel, _ = CreateGrimPanel(RightPanel)
	DetailPanel.Size = UDim2.new(1, 0, 0, 140)
	DetailPanel.Position = UDim2.new(0, 0, 1, -140)
	DetailPanel.Visible = false

	local DTitle = CreateSharpLabel(DetailPanel, "", UDim2.new(1, -20, 0, 35), Enum.Font.GothamBlack, UIHelpers.Colors.TextWhite, 20)
	DTitle.Position = UDim2.new(0, 15, 0, 10)
	DTitle.TextXAlignment = Enum.TextXAlignment.Left

	local DDesc = CreateSharpLabel(DetailPanel, "", UDim2.new(0.55, 0, 0, 60), Enum.Font.GothamMedium, UIHelpers.Colors.TextMuted, 12)
	...
	StatCardContainer.Size = UDim2.new(0.4, 0, 0, 60)
	StatCardContainer.Position = UDim2.new(0.6, 0, 0, 35)
	...
	DCost.Position = UDim2.new(0.68, 0, 1, -35)
	...
	DReq.Position = UDim2.new(0, 15, 1, -30)
	DReq.TextXAlignment = Enum.TextXAlignment.Left

	local UnlockBtn, UBtnStroke = CreateSharpButton(DetailPanel, "UNLOCK", UDim2.new(0.25, 0, 0, 40), Enum.Font.GothamBlack, 14)
	UnlockBtn.Position = UDim2.new(0.98, 0, 1, -50)
	...
	local function UpdateUI()
		...
		if PointsLabel then PointsLabel.Text = "AVAILABLE POINTS: " .. pts end
		...
		if SelectedNodeId and DetailPanel.Visible and type(GameData) == "table" and GameData.PrestigeNodes then
			...
			elseif not hasReq then 
				DReq.Text = "REQUIRES: " .. GameData.PrestigeNodes[node.Req].Name
				...
				UnlockBtn.TextColor3 = Color3.fromHex(node.Color:gsub("#", ""))
				UBtnStroke.Color = Color3.fromHex(node.Color:gsub("#", ""))
	...
	if type(GameData) == "table" and GameData.PrestigeNodes then
		for id, node in pairs(GameData.PrestigeNodes) do
			...
			local iconLbl = CreateSharpLabel(btn, "", UDim2.new(1, 0, 1, 0), Enum.Font.GothamBlack, UIHelpers.Colors.TextMuted, 16)
			iconLbl.ZIndex = 6
			iconLbl.Rotation = -45 
			...
			btn.MouseButton1Click:Connect(function()
				...
				DTitle.Text = node.Name
				DTitle.TextColor3 = Color3.fromHex(node.Color:gsub("#", ""))
				DDesc.Text = node.Desc
				DCost.Text = node.Cost .. " PTS"
	...
	local function RenderLinesAndNodes()
		...
		if type(GameData) == "table" and GameData.PrestigeNodes then
			for id, node in pairs(GameData.PrestigeNodes) do
				...
				if gui and gui.Btn then gui.Btn.Position = node.Pos end
...
local function BuildInheritanceTab(parentFrame, cachedTooltipMgr)
	...
	MainScroll:GetPropertyChangedSignal("Visible"):Connect(function() 
		if not MainScroll.Visible then 
			isAutoRolling.Titan = false
			isAutoRolling.Clan = false 
		end 
	end)

	local titleLayout = Instance.new("UIListLayout", MainScroll)
	titleLayout.SortOrder = Enum.SortOrder.LayoutOrder
	titleLayout.Padding = UDim.new(0, 15)
	titleLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center

	local pad = Instance.new("UIPadding", MainScroll)
	pad.PaddingTop = UDim.new(0, 10)
	pad.PaddingBottom = UDim.new(0, 30)

	local Title = CreateSharpLabel(MainScroll, "THE PATHS (INHERITANCE)", UDim2.new(1, 0, 0, 40), Enum.Font.Garamond, UIHelpers.Colors.Gold, 26)
	Title.LayoutOrder = 1
	...
	local function CreateGachaPanel(gType, order)
		local Panel, _ = CreateGrimPanel(MainScroll)
		Panel.Size = UDim2.new(0.95, 0, 0, 600)
		...
		local SList = Instance.new("UIListLayout", ListContainer)
		...
		if gType == "Titan" and type(TitanData) == "table" and TitanData.Titans then
			...
			for _, drop in ipairs(sortedTitans) do
				...
				local card, _ = CreateGrimPanel(ListContainer)
				...
				local titleLbl = CreateSharpLabel(card, "<b><font color='" .. cColor .. "'>[" .. drop.Rarity .. "] " .. drop.Name .. "</font></b><font color='#888888'>" .. pctStr .. "</font>", UDim2.new(1, -20, 0, 20), Enum.Font.GothamBold, Color3.fromRGB(230, 230, 230), 15)
				titleLbl.Position = UDim2.new(0, 10, 0, 10)
				titleLbl.TextXAlignment = Enum.TextXAlignment.Left
				titleLbl.RichText = true

				local statsArea = Instance.new("Frame", card)
				statsArea.Size = UDim2.new(1, -20, 0, 35)
				statsArea.Position = UDim2.new(0, 10, 0, 35)
				statsArea.BackgroundTransparency = 1
				local saLayout = Instance.new("UIListLayout", statsArea)
				saLayout.FillDirection = Enum.FillDirection.Horizontal
				saLayout.Padding = UDim.new(0, 8)
				...
				for _, st in ipairs(statsOrder) do
					local sBox, _ = CreateGrimPanel(statsArea)
					sBox.Size = UDim2.new(0, 45, 1, 0)
					local sName = CreateSharpLabel(sBox, st.Name, UDim2.new(1, 0, 0.5, 0), Enum.Font.GothamMedium, UIHelpers.Colors.TextMuted, 9)
					local sVal = CreateSharpLabel(sBox, st.Val, UDim2.new(1, 0, 0.5, 0), Enum.Font.GothamBlack, GetRankColor(st.Val), 14)
					sVal.Position = UDim2.new(0, 0, 0.5, 0)
				end
			end
		elseif type(TitanData) == "table" and TitanData.ClanWeights then
			...
			for _, drop in ipairs(sortedClans) do
				...
				elseif drop.Weight <= 15.0 then rarityTag = "Rare" end
				...
				local titleLbl = CreateSharpLabel(card, "<b><font color='" .. cColor .. "'>[" .. rarityTag .. "] " .. drop.Name .. "</font></b><font color='#888888'>" .. pctStr .. "</font>", UDim2.new(1, -20, 0, 20), Enum.Font.GothamBold, Color3.fromRGB(230, 230, 230), 15)
				titleLbl.Position = UDim2.new(0, 10, 0, 10)
				titleLbl.TextXAlignment = Enum.TextXAlignment.Left
				titleLbl.RichText = true

				local descLbl = CreateSharpLabel(card, buffText, UDim2.new(1, -20, 0, 30), Enum.Font.GothamMedium, Color3.fromRGB(150, 255, 150), 11)
		...
		task.delay(0.05, function() Panel.Size = UDim2.new(0.95, 0, 0, SList.AbsoluteContentSize.Y + 280) end)

		local BottomArea = Instance.new("Frame", Panel)
		BottomArea.Size = UDim2.new(1, 0, 0, 210)
		BottomArea.Position = UDim2.new(0, 0, 1, -220)
		BottomArea.BackgroundTransparency = 1

		local ResultLbl = CreateSharpLabel(BottomArea, "Current: None", UDim2.new(1, 0, 0, 30), Enum.Font.GothamBlack, UIHelpers.Colors.TextWhite, 20)
		ResultLbl.RichText = true

		local StorageArea = Instance.new("Frame", BottomArea)
		StorageArea.Size = UDim2.new(0.9, 0, 0, 50)
		StorageArea.Position = UDim2.new(0.05, 0, 0, 35)
		StorageArea.BackgroundTransparency = 1
		local sg = Instance.new("UIGridLayout", StorageArea)
		sg.CellSize = UDim2.new(0.15, 0, 1, 0)
		sg.CellPadding = UDim2.new(0.02, 0, 0, 0)
		sg.HorizontalAlignment = Enum.HorizontalAlignment.Center

		local storageBtns = {}
		for i = 1, 6 do
			local sBtn, stroke = CreateSharpButton(StorageArea, "Empty", UDim2.new(1, 0, 1, 0), Enum.Font.GothamBold, 9)
		...
		local PityLbl = CreateSharpLabel(BottomArea, "PITY: 0 / 100", UDim2.new(1, 0, 0, 20), Enum.Font.GothamBold, Color3.fromRGB(200, 150, 255), 14)
		PityLbl.Position = UDim2.new(0, 0, 0, 100)

		local RollActions = Instance.new("Frame", BottomArea)
		RollActions.Size = UDim2.new(0.9, 0, 0, 50)
		RollActions.Position = UDim2.new(0.05, 0, 0, 130)
		RollActions.BackgroundTransparency = 1
		local raLayout = Instance.new("UIListLayout", RollActions)
		raLayout.FillDirection = Enum.FillDirection.Horizontal
		raLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center
		raLayout.Padding = UDim.new(0.03, 0)

		local labelPrefix = (gType == "Titan") and "Serum" or "Vial"
		local RollBtn, rStroke = CreateSharpButton(RollActions, "ROLL (1x " .. labelPrefix .. ")\nOwned: 0", UDim2.new(0.3, 0, 1, 0), Enum.Font.GothamBlack, 10)
		local PremiumRollBtn, pStroke = CreateSharpButton(RollActions, "N/A", UDim2.new(0.3, 0, 1, 0), Enum.Font.GothamBlack, 10)
		...
		local AutoRollBtn, aStroke = CreateSharpButton(RollActions, "ROLL TILL LEGENDARY+", UDim2.new(0.3, 0, 1, 0), Enum.Font.GothamBlack, 10)
		AutoRollBtn.TextScaled = true
		local arCon = Instance.new("UITextSizeConstraint", AutoRollBtn)
		arCon.MaxTextSize = 10
		...
		RollBtn.MouseButton1Click:Connect(function()
			...
			if count > 0 then
				...
				currentRollSeq[gType] = currentRollSeq[gType] + 1
				local seq = currentRollSeq[gType]
		...
		for i = 1, 6 do
			...
			local function styleVaultSlot(storeObj, storedName, hasVault, isTitanType)
				...
				if i > 3 and not hasVault then 
					btn.Text = "🔒"
					storeObj.Stroke.Color = Color3.fromRGB(80, 40, 40)
					...
					if storedName ~= "None" then
						local rarity = "Common"
						if isTitanType then
							local tData = type(TitanData) == "table" and TitanData.Titans and TitanData.Titans[storedName]
							...
							if string.find(storedName, "Awakened") then 
								...
								elseif weight <= 15.0 then rarity = "Rare" end
							end
						end
						local cColor = Color3.fromHex((RarityColors[rarity] or "#FFFFFF"):gsub("#",""))
						storeObj.Stroke.Color = cColor
						btn.TextColor3 = Color3.fromRGB(230, 230, 230)
					else 
						storeObj.Stroke.Color = UIHelpers.Colors.BorderMuted
	...
	Network.GachaResult.OnClientEvent:Connect(function(gType, resultName, resultRarity)
		if resultName == "Error" then 
			isRolling[gType] = false; isAutoRolling[gType] = false
			UpdateUI()
			if VFXManager and type(VFXManager.PlaySFX) == "function" then VFXManager.PlaySFX("Error", 1) end 
			return 
		end
		...
		local names = {}
		if gType == "Titan" and type(TitanData) == "table" and TitanData.Titans then 
			for tName, _ in pairs(TitanData.Titans) do table.insert(names, tName) end 
		elseif type(TitanData) == "table" and TitanData.ClanWeights then 
			for cName, _ in pairs(TitanData.ClanWeights) do table.insert(names, cName) end 
		end
		...
		if isAutoRolling[gType] and MainScroll.Visible then
			if resultRarity == "Legendary" or resultRarity == "Mythical" or resultRarity == "Transcendent" then
				...
				if (player:GetAttribute(attrReq) or 0) > 0 then
					...
					isAutoRolling[gType] = false; isRolling[gType] = false
...
local function BuildBountiesTab(parentFrame)
	...
	local function CreateBountyCard(bountyId, titlePrefix)
		local card, strk = CreateGrimPanel(ScrollContainer)
		card.Size = UDim2.new(0.95, 0, 0, 95)

		local titleLbl = CreateSharpLabel(card, titlePrefix, UDim2.new(1, -20, 0, 30), Enum.Font.GothamBlack, UIHelpers.Colors.Gold, 16)
		titleLbl.Position = UDim2.new(0, 15, 0, 10)
		titleLbl.TextXAlignment = Enum.TextXAlignment.Left

		local progBarBG = Instance.new("Frame", card)
		progBarBG.Size = UDim2.new(0.9, 0, 0, 16)
		progBarBG.Position = UDim2.new(0.5, 0, 0, 45)
		progBarBG.AnchorPoint = Vector2.new(0.5, 0)
		progBarBG.BackgroundColor3 = Color3.fromRGB(10, 10, 12)
		local pbStroke = Instance.new("UIStroke", progBarBG)
		pbStroke.Color = Color3.fromRGB(50, 50, 60)
		pbStroke.Thickness = 2
		...
		local progText = CreateSharpLabel(card, "0 / 1", UDim2.new(1, 0, 0, 20), Enum.Font.GothamBold, UIHelpers.Colors.TextWhite, 12)
		progText.Position = UDim2.new(0, 0, 0, 65)

		local actionBtn, actStroke = CreateSharpButton(card, "IN PROGRESS", UDim2.new(0, 140, 0, 36), Enum.Font.GothamBlack, 12)
		...
		actionBtn.MouseButton1Click:Connect(function() 
			if actionBtn.Active and actionBtn.Text == "CLAIM REWARD" then Network:WaitForChild("ClaimBounty"):FireServer(bountyId) end 
		end)
		player.AttributeChanged:Connect(function(attr) if string.match(attr, "^" .. bountyId) then UpdateCard() end end)
		UpdateCard()
	end
...
function MobileHeroMenu.Initialize(parentFrame, tooltipMgr)
	...
	pSubNav.Size = UDim2.new(1, 0, 0, 50)
	pSubNav.BackgroundTransparency = 1
	pSubNav.ScrollBarThickness = 0
	pSubNav.ScrollingDirection = Enum.ScrollingDirection.X

	local pNavLayout = Instance.new("UIListLayout", pSubNav)
	pNavLayout.FillDirection = Enum.FillDirection.Horizontal
	pNavLayout.HorizontalAlignment = Enum.HorizontalAlignment.Left
	pNavLayout.VerticalAlignment = Enum.VerticalAlignment.Center
	pNavLayout.Padding = UDim.new(0, 10)
	pNavLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() 
		pSubNav.CanvasSize = UDim2.new(0, pNavLayout.AbsoluteContentSize.X + 20, 0, 0) 
	end)

	local pContent = Instance.new("Frame", parentFrame)
	pContent.Size = UDim2.new(1, 0, 1, -50)
	pContent.Position = UDim2.new(0, 0, 0, 50)
	...
	for i, tabName in ipairs(subTabs) do
		local btn, stroke = CreateSharpButton(pSubNav, tabName, UDim2.new(0, 130, 0, 35), Enum.Font.GothamBold, 12)
		btn.TextColor3 = (i==1) and UIHelpers.Colors.Gold or UIHelpers.Colors.TextMuted
		stroke.Color = (i==1) and UIHelpers.Colors.Gold or UIHelpers.Colors.BorderMuted
