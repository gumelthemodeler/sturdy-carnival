...
function MobileSquadsTab.Initialize(parentFrame)
	...
	local function OpenInventorySelection(slotId)
		...
		local function ScanItems(dictionary)
			for itemName, _ in pairs(dictionary) do
				...
				if count > 0 then
					foundAny = true
					local btn, stroke = CreateSharpButton(InvScroll, count .. "x " .. itemName, UDim2.new(1, -10, 0, 45), Enum.Font.GothamBold, 12)
					btn.ZIndex = 53
	...
	local LeaveDisbandBtn, ldStroke = CreateSharpButton(HeaderPanel, "LEAVE SQUAD", UDim2.new(0, 100, 0, 25), Enum.Font.GothamBlack, 10, "#FF5555")
	LeaveDisbandBtn.Position = UDim2.new(1, -10, 0, 10); LeaveDisbandBtn.AnchorPoint = Vector2.new(1, 0); LeaveDisbandBtn.TextColor3 = Color3.fromRGB(255, 85, 85)
	...
	RosterPanel.Size = UDim2.new(1, 0, 0, 480)
	...
	RosterList.Size = UDim2.new(1, -20, 1, -45)
	RosterList.Position = UDim2.new(0, 10, 0, 35)
	...
	rlLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() RosterList.CanvasSize = UDim2.new(0,0,0, rlLayout.AbsoluteContentSize.Y + 10) end)
	...
	local VaultGrid = Instance.new("ScrollingFrame", VaultActiveView)
	VaultGrid.Size = UDim2.new(1, 0, 1, -80); VaultGrid.Position = UDim2.new(0, 0, 0, 80); VaultGrid.BackgroundTransparency = 1; VaultGrid.ScrollBarThickness = 4; VaultGrid.BorderSizePixel = 0
	local vgLayout = Instance.new("UIGridLayout", VaultGrid); vgLayout.CellSize = UDim2.new(0, 80, 0, 80); vgLayout.CellPadding = UDim2.new(0, 15, 0, 15); vgLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center
	vgLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() VaultGrid.CanvasSize = UDim2.new(0, 0, 0, vgLayout.AbsoluteContentSize.Y + 10) end)
	...
	local function UpdateSquadUI()
		...
		if mySquad and mySquad ~= "" and mySquad ~= "None" then
			...
			if isLeader then
				...
				local reqBtn, rStrk = CreateSharpButton(InSquadView, "VIEW REQUESTS", UDim2.new(0, 100, 0, 25), Enum.Font.GothamBlack, 10)
				reqBtn.Name = "ManageReqsBtn"
				reqBtn.Position = UDim2.new(1, -120, 0, 10)
				...
				reqBtn.MouseButton1Click:Connect(function()
					...
					for _, req in ipairs(reqs) do
						...
						local accBtn = CreateSharpButton(rCard, "ACCEPT", UDim2.new(0, 60, 0, 30), Enum.Font.GothamBlack, 10, "#55FF55")
						accBtn.Position = UDim2.new(1, -70, 0.5, 0); accBtn.AnchorPoint = Vector2.new(1, 0.5)

						local denBtn = CreateSharpButton(rCard, "DENY", UDim2.new(0, 50, 0, 30), Enum.Font.GothamBlack, 10, "#FF5555")
