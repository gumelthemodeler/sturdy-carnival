...
function MobileCombatBuilder.Build(masterScreenGui, player)
	...
	-- Original PC layout scale, to be dynamically shrunken by UIScale in the controller
	...
	-- THE FIX: Zero Yield. Instant Image Loading. 
	...
	local function BuildLimb(name, targetId, size, pos, hoverText, baseColor)
		...
		-- Used Touch controls for Mobile specific
		limb.InputBegan:Connect(function()
		...
		limb.InputEnded:Connect(function()
			strk.Color = baseColor
			strk.Thickness = 2
	...
	GUI.ContinueHint = UIHelpers.CreateLabel(GUI.DialogueBox, "Tap anywhere to continue ▼", UDim2.new(1, -20, 0, 20), Enum.Font.GothamBold, Color3.fromRGB(150, 150, 150), 12)
